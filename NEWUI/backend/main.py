...
- "Decision period (n)": Number of time periods since the last maintenance intervention
- The model decides WHEN to intervene (at which decision period) and
...
def ReliabilityLP(Code, K, C, U, alpha, cp, cc, ct, cr, cu, co,
    ...
    represents the long-run fraction of time the system is in signal state s, at decision
    ...
    # Constraint 1: Flow balance at (not-red, decision period 0)
    ...
    # Constraint 2: Transition from decision period 0 to decision period 1
    ...
    # Constraints 3 & 4: Flow balance for decision periods 2 through U
    ...
    # Constraint: Cannot reach red before decision period K
    for n in range(K):
        for a in range(C + 1):
            row = np.zeros(num_vars)
            row[_var_index(1, n, a, U, C)] = 1.0
    ...
    if Code in [1, 2, 3]:
        # Only intervene at decision period K-1
        for n in range(K - 1):
            for a in range(1, C + 1):
                row = np.zeros(num_vars)
                row[_var_index(0, n, a, U, C)] = 1.0
...
async def optimize(params: OptimizeRequest):
    ...
    runs the optimization model, and returns corrected results including:
    - probNotRed: Probability of NOT being in red state
    - maxInterventionLevel: Maximum yellow intervention level (preventive only)
    - avgInterventionLevel: Average intervention level (preventive only)
    - optimalPolicy: Policy array for yellow/not-red decisions
    - tableRows: Clean table data for frontend display
    """
    try:
        eps = 0.01
        ...
        # Sum all probabilities in red state (s=1)
        prob_red = float(np.sum(SolutionMat[1, :, :]))
        probNotRed = 1.0 - prob_red

        # 2. maxInterventionLevel: Maximum yellow/not-red decision period with preventive maintenance
        # Only check not-red state (s=0) with action a >= 1
        maxInterventionLevel = 0
        threshold = 1e-6
        for n in range(U + 1):
            preventive_prob = float(np.sum(SolutionMat[0, n, 1:params.C + 1]))
            if preventive_prob > threshold:
                maxInterventionLevel = n

        # 3. avgInterventionLevel: Average intervention level (preventive only, not-red state)
        # Only include SolutionMat[0, n, a] for a >= 1
        weighted_sum = 0.0
        ...
        avgInterventionLevel = (
            weighted_sum / total_preventive_prob if total_preventive_prob > threshold else 0.0
        )

        # 4. optimalPolicy: Build policy array for yellow/not-red decisions only
        optimalPolicy = []
        for n in range(U + 1):
            preventive_prob = float(np.sum(SolutionMat[0, n, 1:params.C + 1]))
            if preventive_prob > threshold:
        ...
        # 5. costByState: Expected cost at each decision period
        costByState = []
        for n in range(U + 1):
            # Cost from not-red state
            cost_not_red = float(np.sum(SolutionMat[0, n, :])) * params.cp
            # Note: We don't include red state costs in this visualization
            # since the table only shows yellow/not-red decisions
            costByState.append(cost_not_red)

        # 6. tableRows: Clean table data for frontend
        tableRows = []
        for n in range(U + 1):
            preventive_prob = float(np.sum(SolutionMat[0, n, 1:params.C + 1]))
            no_action_prob = float(SolutionMat[0, n, 0])
            ...
            # Calculate expected cost at this decision period
            expected_cost = 0.0
            if preventive_prob > threshold:
                expected_cost = params.cp + params.ct * params.C / 2  # Approximate

            # Calculate state probability (not-red state at this n)
            ...
            # Stop after first preventive intervention for cleaner table
            if preventive_prob > threshold:
                break

        # Build state description vectors (for compatibility)
        DetSSize = params.K + 1
        StateSpacesSize = DetSSize ** params.C
        state_descriptions = []
        for n in range(min(StateSpacesSize, 100)):
            desc = StateDesc(n, params.C, DetSSize)
            state_descriptions.append(desc.tolist())

        # ===== Return corrected results to frontend =====
        return {
            "probNotRed": round(probNotRed, 6),
            "maxInterventionLevel": maxInterventionLevel,
            "avgInterventionLevel": round(avgInterventionLevel, 4),
