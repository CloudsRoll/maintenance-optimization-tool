...
export function ResultsDashboard({ result, onReset }: ResultsDashboardProps) {
  const costData = result.costByState.map((cost, index) => ({
    id: `cost-${index}`,
    state: `State ${index}`,
    cost: parseFloat(cost.toFixed(2)),
  }));

  const policyData = result.optimalPolicy.map((action, index) => ({
    id: `policy-${index}`,
    state: `S${index}`,
    action: action === 0 ? 'Do Nothing' : action === 1 ? 'Preventive' : 'Corrective',
    actionValue: action,
  }));
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-card border border-border rounded-lg p-6">
          <h3 className="mb-4">Optimal Policy by State</h3>
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          <h3 className="mb-4">Expected Cost by State</h3>
