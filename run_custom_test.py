...
def ReliabilityLP(Code,K,C,U,alpha,cp,cc,cr,cu,co):
    ...
    lp.addConstr(gp.quicksum(P[0,0,:]) - gp.quicksum(P[s,n,a] for s in range(2) for n in range(U+2) for a in range(1,C+1)) - CounterStuckP*P[0,0,0] == 0)
    lp.addConstr(gp.quicksum(P[0,1,:]) - (1-CounterStuckP)*P[0,0,0] == 0)

    for n in range(2,U+2):
        lp.addConstr(gp.quicksum(P[0,n,:]) - ObsSTransProbMat[n-1,0]*P[0,n-1,0] == 0)
        lp.addConstr(gp.quicksum(P[1,n,:]) - ObsSTransProbMat[n-1,1]*P[0,n-1,0] == 0)
